...
["Pressão da Torcida",p=>{let a=p.roster.filter(x=>!x.starter&&x.source!==p.country);if(!a.length)return"Nenhum reserva capturado disponível.";let x=a[Math.floor(Math.random()*a.length)];p.roster=p.roster.filter(y=>y.id!==x.id);return`${x.name} rescindiu e voltou para ${x.source}.`; }]
];
...
function v4CreateRoster(country){
  ...
  const list=source.map((name,i)=>{
    let nativePos=POSITION_V4[name]||(["G","DEF","DEF","DEF","MID","MID","MID","ATT","ATT","ATT","ATT"][i]);
    ...
    return v4MakePlayer(name,ovr,nativePos,country,true);
  });
  const reserves=[
...
function advanceTurnV4(){Object.values(STATE.effects).forEach(e=>Object.keys(e).forEach(k=>{if(e[k].turns<90){e[k].turns--;if(e[k].turns<=0)delete e[k]}}));STATE.players.forEach(p=>p.roster.forEach(x=>{if(x.injured>0)x.injured--;x.yellow=0;x.red=false}));let guard=0;do{STATE.current=(STATE.current+1)%STATE.players.length;guard++}while(STATE.players[STATE.current].eliminated&&guard<=STATE.players.length);if(STATE.current===0)STATE.turn++;renderGame();checkVictoryV4()}

function manageTeam(){const p=current();if(v4Effect(p,"noSubs"))return toast("Escalação bloqueada nesta rodada.");v4AutoSub(p,null);const c=v4Counts(p);document.getElementById("modalContent").innerHTML=`<h2>👥 ${p.country} — ${c.G}/${c.DEF}/${c.MID}/${c.ATT}</h2><div class="formations">${["442","433","353","CUSTOM"].map(f=>`<button class="formation ${p.formation===f?"active":""}" data-form="${f}">${FORMATIONS_V4[f].label}</button>`).join("")}</div><div id="customFormation"></div><div class="rosters"><div class="roster-box"><h3>TITULARES</h3>${["G","DEF","MID","ATT"].map(pos=>`<div class="position-title">${pos==="G"?"🧤 GOLEIROS":pos==="DEF"?"🛡️ DEFENSORES":pos==="MID"?"🎯 MEIAS":"⚡ ATACANTES"}</div>${p.roster.filter(x=>x.starter&&x.pos===pos).map(playerRowV4).join("")}`).join("")}</div><div class="roster-box"><h3>BANCO</h3>${p.roster.filter(x=>!x.starter).map(playerRowV4).join("")}<button id="sellFromTeam" class="secondary wide">💸 VENDER JOGADOR</button></div></div>`;showModal();bindRowsV4();document.querySelectorAll("[data-form]").forEach(b=>b.onclick=()=>chooseFormationV4(p,b.dataset.form));if(p.formation==="CUSTOM")customFormationUI(p)}
...
function playerRowV4(x){const opts=(ALLOWED_V4[x.nativePos]||[x.nativePos]).map(v=>`<option value="${v}" ${x.pos===v?"selected":""}>${POS[v]}</option>`).join("");return `<div class="player-row"><span>${x.nativePos==="G"?"🧤":"⚽"}</span><span><b>${x.name}</b><br><small>${x.starter?"TITULAR":"BANCO"} • ${POS[x.nativePos]}${x.injured?" • LESIONADO":""}</small></span><select data-pos-v4="${x.id}" ${x.injured||x.red?"disabled":""}>${opts}</select><b>${x.ovr}</b><span>${x.source}</span><button data-toggle-v4="${x.id}" ${x.injured||x.red?"disabled":""}>${x.starter?"BANCO":"ENTRAR"}</button></div>`}
function bindRowsV4(){document.querySelectorAll("[data-toggle-v4]").forEach(b=>b.onclick=()=>{const p=current(),x=p.roster.find(y=>y.id===b.dataset.toggleV4);if(!x)return;if(!x.starter&&p.roster.filter(y=>y.starter).length>=11)return toast("Máximo de 11 titulares.");x.starter=!x.starter;manageTeam()});document.querySelectorAll("[data-pos-v4]").forEach(s=>s.onchange=()=>{const x=current().roster.find(y=>y.id===s.dataset.posV4);if(!x)return;const v=s.value;if(v==="G"&&x.nativePos!=="G"&&!v4Effect(current(),"freePosition"))return toast("Somente goleiros podem jogar no gol.");x.pos=v})}

function market(){const p=current();const available=STATE.market.filter(x=>!x.sold);document.getElementById("modalContent").innerHTML=`<h2>💰 MERCADO DE TRANSFERÊNCIAS</h2><p>Saldo: <b class="money">$${p.money}M</b>. Só entram jogadores fortes ou famosos; o mercado vivo recebe jogadores 87+ que não foram capturados.</p><div class="market-grid">${available.map(x=>{const price=Math.max(35,Math.round(x.ovr*2.1));return `<div class="market-card"><span><b>${x.name}</b><br><small>${POS[x.pos]} • OVR ${x.ovr}${x.source&&x.source!=="Mercado"?` • ${x.source}`:""}</small></span><strong class="money">$${price}M</strong><button data-buy-v4="${x.id}" ${p.money<price?"disabled":""}>CONTRATAR</button></div>`}).join("")||"<p>Nenhum jogador disponível.</p>"}</div><button id="sellBtnV4" class="secondary wide">💸 VENDER JOGADOR</button>`;showModal();document.querySelectorAll("[data-buy-v4]").forEach(b=>b.onclick=()=>{const x=STATE.market.find(y=>y.id===b.dataset.buyV4);if(!x)return;const price=Math.max(35,Math.round(x.ovr*2.1));if(p.money<price)return toast("Dinheiro insuficiente.");p.money-=price;x.sold=true;const pl=v4MakePlayer(x.name,x.ovr,x.pos,p.country,false,x.id+"_owned");pl.market=true;p.roster.push(pl);STATE.market=STATE.market.filter(y=>y.id!==x.id);addLog(`💰 ${p.country} contratou ${x.name} por $${price}M.`);market()});document.getElementById("sellBtnV4").onclick=()=>chooseSellV4()}
function chooseSellV4(){const p=current();document.getElementById("modalContent").innerHTML=`<h2>💸 VENDER JOGADOR</h2><p>Todo jogador exige confirmação antes da venda.</p>${p.roster.map(x=>`<div class="player-row"><span>${x.nativePos==="G"?"🧤":"⚽"}</span><span><b>${x.name}</b><br><small>OVR ${x.ovr} • ${POS[x.nativePos]}</small></span><span></span><b>$${sellValueV4(x)}M</b><span></span><button data-sell-v4="${x.id}">VENDER</button></div>`).join("")}`;document.querySelectorAll("[data-sell-v4]").forEach(b=>b.onclick=()=>confirmSellV4(b.dataset.sellV4))}
function sellValueV4(x){return Math.max(5,Math.round(x.ovr*.55+(x.market?10:0)))}
function confirmSellV4(id){const p=current(),x=p.roster.find(y=>y.id===id);if(!x)return;document.getElementById("modalContent").innerHTML=`<div class="event-box"><div class="event-title warn">⚠️ TEM CERTEZA QUE DESEJA VENDER?</div><h2>${x.name}</h2><p>OVR ${x.ovr} • ${POS[x.nativePos]}</p><p>Você receberá <b class="money">$${sellValueV4(x)}M</b>.</p><button id="cancelSellV4" class="secondary">CANCELAR</button> <button id="confirmSellV4" class="primary">VENDER</button></div>`;document.getElementById("cancelSellV4").onclick=chooseSellV4;document.getElementById("confirmSellV4").onclick=()=>{p.roster=p.roster.filter(y=>y.id!==x.id);p.money+=sellValueV4(x);addLog(`💸 ${p.country} vendeu ${x.name}.`);market()}}
...
document.getElementById("marketBtn").onclick=market;
