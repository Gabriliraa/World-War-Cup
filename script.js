...
function manageTeam(){
 const p=current();if(v4Effect(p,"noSubs"))return toast("Escalação bloqueada nesta rodada.");v4AutoSub(p,null);const c=v4Counts(p);
 const row=(x,type)=>`<tr class="${x.injured||x.red?"injured":""}"><td><input type="radio" name="${type}" data-sub-${type}="${x.id}" ${x.injured||x.red?"disabled":""}></td><td><b>${x.name}</b></td><td>${POS[x.nativePos]}</td><td>${POS[x.pos]}</td><td><b>${x.ovr}</b></td><td>${x.injured?"🩹 Lesionado":x.red?"🟥 Expulso":type==="out"?"Em campo":"Disponível"}</td></tr>`;
 document.getElementById("modalContent").innerHTML=`<h2>👥 ${p.country} — Gerenciamento do elenco</h2><p class="muted">Selecione um titular para sair e um reserva para entrar. A substituição preserva a posição tática e impede escalações inválidas.</p><div class="formations">${["442","433","353","CUSTOM"].map(f=>`<button class="formation ${p.formation===f?"active":""}" data-form="${f}">${FORMATIONS_V4[f].label}</button>`).join("")}</div><div id="customFormation"></div><div class="rosters"><div class="roster-box"><h3>TITULARES — selecione quem sai</h3><table class="stats-table"><thead><tr><th>SAIR</th><th>Jogador</th><th>Posição</th><th>Função</th><th>OVR</th><th>Status</th></tr></thead><tbody>${p.roster.filter(x=>x.starter).map(x=>row(x,"out")).join("")}</tbody></table></div><div class="roster-box"><h3>BANCO — selecione quem entra</h3><table class="stats-table"><thead><tr><th>ENTRAR</th><th>Jogador</th><th>Posição</th><th>Função</th><th>OVR</th><th>Status</th></tr></thead><tbody>${p.roster.filter(x=>!x.starter).map(x=>row(x,"in")).join("")}</tbody></table><button id="confirmSubV4" class="primary wide">🔄 FAZER SUBSTITUIÇÃO</button><button id="sellFromTeam" class="secondary wide">💸 VENDER JOGADOR</button></div></div>`;
 showModal();document.getElementById("confirmSubV4").onclick=applySubstitutionV4;document.getElementById("sellFromTeam").onclick=chooseSellV4;document.querySelectorAll("[data-form]").forEach(b=>b.onclick=()=>chooseFormationV4(p,b.dataset.form));if(p.formation==="CUSTOM")customFormationUI(p);
}
