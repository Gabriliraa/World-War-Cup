...
function sellSelectedV4(){const id=document.querySelector('input[name="sellPlayerV4"]:checked')?.value;if(!id)return toast("Selecione um jogador para vender.");const p=current(),x=p.roster.find(y=>y.id===id);if(!x)return toast("Jogador indisponível.");const sale=sellValueV4(x);p.roster=p.roster.filter(y=>y.id!==id);p.money+=sale;if(x.ovr>=88){const marketPrice=Math.ceil(sale*4/3);STATE.market=STATE.market.filter(y=>y.sourcePlayerId!==x.id);STATE.market.push({id:`sale_${x.id}_${Date.now()}`,name:x.name,ovr:x.ovr,pos:x.nativePos,source:p.country,sourcePlayerId:x.id,sold:false,marketPrice});addLog(`💸 ${p.country} vendeu ${x.name} por $${sale}M; ele entrou no mercado por $${marketPrice}M.`)}else addLog(`💸 ${p.country} vendeu ${x.name} por $${sale}M.`);toast(`${x.name} foi vendido por $${sale}M.`);market()}
